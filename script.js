...
async function exportToJSON() {
    try {
        ...
        const fileName = `math_library_${timestamp}.json`;
        ...
        showStorageStatus(`✅ Файл "${fileName}" успішно експортовано!`, 'success');
        
    } catch (error) {
        console.error('Помилка експорту:', error);
        showStorageStatus('❌ Помилка експорту даних', 'error');
    }
}
...
async function importFromJSON(event) {
    ...
    if (!file.name.endsWith('.json')) {
        showStorageStatus('❌ Файл має бути у форматі JSON', 'error');
        event.target.value = '';
        return;
    }
    ...
    try {
        ...
        if (!Array.isArray(materials)) {
            throw new Error('Некоректна структура файлу');
        }
        
        if (!confirm(`Ви впевнені, що хочете імпортувати ${materials.length} матеріалів? Поточні дані будуть перезаписані.`)) {
            event.target.value = '';
            return;
        }
        
        showStorageStatus('🔄 Імпорт даних...', 'info');
        ...
        for (const material of materials) {
            try {
                ...
                errors.push(material.title || material.fileName || 'Невідомий матеріал');
            }
        }
        ...
        let message = `✅ Успішно імпортовано ${importedCount} матеріалів`;
        if (errors.length > 0) {
            message += `<br>❌ Помилки: ${errors.length} матеріалів не імпортовано`;
        }
        
        showStorageStatus(message, errors.length > 0 ? 'warning' : 'success');
        
    } catch (error) {
        console.error('Помилка імпорту:', error);
        showStorageStatus(`❌ Помилка імпорту: ${error.message}`, 'error');
    }
...
function showStorageStatus(message, type = 'info') {
    // Створюємо або знаходимо контейнер для статусу
    let statusDiv = document.getElementById('storage-status');
    if (!statusDiv) {
        statusDiv = document.createElement('div');
        statusDiv.id = 'storage-status';
        statusDiv.className = 'gist-status';
        document.querySelector('.storage-section').appendChild(statusDiv);
    }
